...
- **merchant-admin list pages are all real now** (`products`, `categories`, `inventory`, `customers`, `orders` — Inventory was added to the sidebar), each a client component on `DataTable`: Products/Categories/Customers have full CRUD via a `Modal` form (`*-form.tsx` next to each `page.tsx`), Inventory edits quantity inline (no create/delete — a row appears when a quantity is first set), Orders is read-only with a line-item detail modal. Customers' phone is create-only in the form (it's the OTP login identity). Content container widened `max-w-4xl` → `max-w-6xl` in both shells.
