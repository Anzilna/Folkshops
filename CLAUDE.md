...
- **`storefront`** is real: catalog (`/`, server-rendered, `status=active` only, category chips + search + pagination via query string), product page (`/products/[id]`, drafts/archived 404), OTP login (`/login`, phone → code, `?next=` return path; the code prints in `core-api`'s log via `ConsoleOtpProvider`), cart (`/cart`, client-side), checkout → `/orders/[id]?placed=1`, order history (`/orders`). `middleware.ts` guards only `/cart` and `/orders` (the rest is public). **The store is fixed per deployment** — `NEXT_PUBLIC_STORE_SLUG` in `.env` (dev-only, sent as `X-Tenant-Id`; production resolves from the hostname), no store-picker cookie like merchant-admin. Anonymous visitors get the store name from the new public `GET /storefront/store`. Products have no images yet — `ProductArt` renders a stable per-name tint + initial. A `fk:cart-changed` window event (`notifyCartChanged()`) tells the header badge to re-count, since add-to-cart doesn't navigate. **Port 3000 may be taken by an unrelated project on this machine** (`ctms/backend` was) — the storefront was verified on 3003 (marketing's port, already in `CORS_ORIGINS`); any port in that list works.
...
**Not built yet anywhere:** marketing site content; product images (no column, no upload — `ProductArt` is the placeholder); `RolesGuard`/`@Roles()` enforcement (see Deferred). No automated browser tests — the admin tables were verified with a throwaway Playwright script + screenshots, not a checked-in suite.
